...
async function main(): Promise<void> {
  ...
  const tsFiles = await glob(`${srcDir}/*.ts`, { posix: true });
  const header = `#!/usr/bin/env node
    // this file is auto-generated by ${path.basename(import.meta.filename)}. Do not edit.
  ...
  const snippets = tsFiles.map((tsFile) => {
    const baseName = path.posix.basename(tsFile, ".ts");
    ...
    const destFile = path.posix.join(destDir, destFileName);
    return `
      import ${baseNameLower} from "../${importFile}";
      const ${jsonName} = JSON.stringify(${baseNameLower}, replacer, 2);
